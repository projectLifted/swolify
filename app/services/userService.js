...
import { signin, postMessage, deleteWallPost, followUser } from '../ducks/userDuck';
...
export function addFollower(newUserId, userId) {
	...
	request.post(`/api/user/following/${userId}`)
		...
		.end((err, user) => {
			...
			console.log(user);
			return store.dispatch(followUser(newUserId));
		});
}
